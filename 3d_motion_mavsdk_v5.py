# Built on 3d_line_mavsdk_v4.py, which can already complete the course
# Mainly miscellaneous updates, which makes the code easier to read and execute
...
async def leader_behavior(leader,target_heading):
    ...
    print("\n<-- Start mission 4, curve path to the second gate -->\n"
    ...
    # Transform target coordinate into leader drone's coordinate
    endpoint7_local_n, endpoint7_local_e = coordinate_transformation(leader.id, WAYPOINT7[1], WAYPOINT7[0], 0)
    ...
    print("\n<-- Start mission 7, move to end gate -->\n"
          ...
          f"Target endpoint: N={endpoint7_local_n:.2f}, E={endpoint7_local_e:.2f}, D={hover_pos['d']:.2f}\n")
    real_local_n, real_local_e, real_local_d, end_heading_7 = await curve_maneuver(leader,
                            endpoint6_local_n, endpoint6_local_e, hover_pos['d'], end_heading_6,
                            endpoint7_local_n, endpoint7_local_e, hover_pos['d'],
                            0.3, 1, 1, 0.5
    )
    ...
    # Transform target coordinate into leader drone's coordinate
    endpoint8_local_n, endpoint8_local_e = coordinate_transformation(leader.id, WAYPOINT8[1], WAYPOINT8[0], 0)
    ...
    print("\n<-- Start mission 8, move to end gate -->\n"
          ...
          f"Target endpoint: N={endpoint8_local_n:.2f}, E={endpoint8_local_e:.2f}, D={hover_pos['d']:.2f}\n")
    end_heading_8 = await straight_maneuver(leader,
                            real_local_n, real_local_e, hover_pos['d'], end_heading_7,
                            endpoint8_local_n, endpoint8_local_e, hover_pos['d'],
                            0.8, 1
    )
    ...
    # Gradually turn to target heading over 3 seconds
    print("Align the leader with the end gate before mission")
    print(f"Leader current heading before turn: {end_heading_8:.1f}°")
    ...
    for i in range(steps + 1):
        ...
        await leader.set_position(
            endpoint8_local_n, endpoint8_local_e, hover_pos['d'], current_heading
        )
    ...
    print("\n<-- Start mission 11, fly back to starting position -->\n"
          f"[Leader] Current local position: N={endpoint8_local_n:.2f}, E={endpoint8_local_e:.2f}, D={hover_pos['d']:.2f}\n"
          f"[Leader] Current heading: {current_heading:.1f}°\n"
          f"Target endpoint: N={gate_front_local_n:.2f}, E={gate_front_local_e:.2f}, D={hover_pos['d']:.2f}\n")
    end_heading_9 = await straight_maneuver(leader,
                            endpoint8_local_n, endpoint8_local_e, hover_pos['d'], current_heading,
    ...
    while True:
        ...
        if phase == 1:
            ...
            if (iteration * UPDATE_RATE) >= WAIT_TIME:
                print(f"[FOLLOWER {i} PHASE 1] Complete, Leader turned, starting positioning")
                phase = 2
        ...
        # PHASE 3: ALTITUDE - Rise to formation altitude
            ...
            if iteration % 10 == 0:
                print(f"[FOLLOWER {i} PHASE 3] Aligning to formation altitude, "
        ...
        # PHASE 4: HEADING - Align heading with leader
            ...
            new_heading = smooth_turn(new_heading)

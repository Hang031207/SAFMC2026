...
"""
2 drones with APF formation + gate collision avoidance
- Formation maintenance during maneuvers
- Gate collision avoidance
- Sequential start to avoid initial collision
"""
...
K_GATE_REPEL       = 8.0  # Gate repulsion strength
D_GATE_SAFE        = 1.0  # Safe distance from gate edges
D_GATE_INFLUENCE   = 4.0  # Gate influence radius
...
def gate_alignment_force(my_pos, gate_center, gate_width):
    ...
    radial_magnitude = K_GATE_REPEL * 0.5 * (1.0/distance - 1.0/D_GATE_INFLUENCE) * (1.0/distance**2)
...
async def drone1_behavior(drone1, drone2):
    ...
    # Mission 1: Move to gate front (lineup position)
    ...
    # Mission 2: Move to waypoint 1 (pass through gate)
    print(f"\n[{drone1.name}] +++ Mission 1: Move to waypoint (19.0, 5.0) +++")
    wp1_local_n, wp1_local_e = coordinate_transformation(
        drone1.id, WAYPOINT1[1], WAYPOINT1[0], 0
    )
    ...
    print(f"[{drone1.name}] +++ Reach waypoint 1, holding... +++")
...
async def drone2_behavior(drone1, drone2):
    ...
    await asyncio.sleep(5)  # Sync with drone1
    ...
    # Mission 1: Move to starting position (0.0, 5.0) - 1.5m from gate front
    ...
    print(f"[{drone2.name}] +++ Reached starting formation, holding for 5s... +++")
    current_pos, _ = await drone2.get_position()
    for _ in range(30):
        await drone2.set_position(current_pos[0], current_pos[1], current_pos[2], end_heading1)
        await asyncio.sleep(UPDATE_RATE)
    
    # Mission 2: Move to waypoint 1
    print(f"\n[{drone2.name}] +++ Mission 1: Move to waypoint (19.0, 5.0) +++")
    wp1_local_n, wp1_local_e = coordinate_transformation(
        drone2.id, WAYPOINT1[1], WAYPOINT1[0], 0
    )

...
#Misc updates: add coordinate transformation function
...
async def straight_maneuver(drone, start_n, start_e, start_d, start_heading, end_n, end_e, end_d, v_default):
    ...
    while travelled_pos < distance:
        ...
        step_h = min(1, 0.3*remaining_h) #turn 1 degree every UPDATE_RATE, which is 0.1s. So 1 second 10 degree turning
        travelled_h += step_h
        if delta_h>0:
...
async def leader_behavior(leader,target_heading):
    ...
    while True:
        await leader.set_position(
            endpoint3_local_n, endpoint3_local_e, hover_pos['d'], current_heading
        )

...
# Based on v6, now want to try incorporate a gate passing algorithm
# Will launch this code in the competition environment, test how the drones behave when they going through the gates
# Warning: Still under development, don't use it
...
# APF Parameters
D_FORMATION        = 1.0 # Desired separation distance (meters) 
D_TOLERANCE        = 0.3 # Acceptable distance error (meters)
K_SPRING           = 5.0 # Spring constant
C_DAMP             = 0.5 # Damping coefficient
MAX_VELOCITY       = 2.5 # Max velocity m/s
...
K_VEL_MATCH        = 3.0 # Velocity matching gain
K_GATE             = 2   # Gate alignment parameters
...
async def straight_maneuver(drone1, drone2, start_n, start_e, start_d, start_heading, end_n, end_e, end_d, v_default, turn_default):
    """
...
def formation_spring_damper_force(my_pos, other_pos, my_velocity, other_velocity, formation_distance, iteration):
    ...
    # Position Control: Spring Force
    distance_error = current_distance - formation_distance
    
    # Adaptive APF parameters:
    ...
    # Velocity Matching: Try to match other's velocity
    ...
    total_force = position_force + velocity_matching_force + damping_force
    ...
    if iteration % 50 == 0:
        ...
        print(f"[FORMATION] Spring force: {position_force}")
        print(f"[FORMATION] Velocity matching force: {velocity_matching_force}")
...
def compute_gate_passing_velocity(drone,my_pos,i):
    """
    Here, my_pos is a local coordinate, but gates coordinates are global
    So need to convert the gates coordinates to local coordinate first
    The i argument means to compute velocity based on which gate, for this code i=1 only
    """
    if i == 1:
        gate_local_n, gate_local_e = coordinate_transformation(drone.id, GATE_1_COOR[1], GATE_1_COOR[0], 0)
    else:
        print("[GATE PASSING]Invalid i argument given!")
        return
    
    gate_local_pos = np.array([gate_local_n, gate_local_e])
    my_pos_2d = my_pos[:2]
    diff = gate_local_pos - my_pos_2d
    current_distance = np.linalg.norm(diff)
    
    if current_distance < 0.01 or current_distance > 2.0:  # Avoid division by zero, and also avoid applying this if too far from gate
        return np.zeros(2)
    
    # To decide whether influenced by gate upper pillar or lower pillar
    if my_pos_2d[0] > gate_local_n:
        k=1
    else: 
        k=-1
    ...
    e_r = diff / current_distance
    angle = math.atan2(e_r[1],e_r[0])
    angle += np.pi/2
    
    # Define rotation matrix: for vector transformation
    if k == 1:
        e_t = vector_rot(e_r, -np.pi/2)
    elif k == -1:
        e_t = vector_rot(e_r, np.pi/2)
    
    gate_passing_force = K_GATE * current_distance * e_t
    
    # Simple model: Directly link gate passing force to the gate aligning velocity
    gate_vel = gate_passing_force
    # Limit horizontal velocity
    vel_horizontal = np.array([gate_vel[0], gate_vel[1], 0.0])
    speed_horizontal = np.linalg.norm(vel_horizontal)
    if speed_horizontal > MAX_VELOCITY:
        scale = MAX_VELOCITY / speed_horizontal
        gate_vel[0] *= scale
...
async def drone1_behavior(drone1,drone2):
    print(f"[{drone1.name}] Waiting for drone2...")
    ...
    print(f"[{drone1.name}] +++ Start 1st Mission: Move to (1.5, 5.0) +++")
    gate_front_local_n, gate_front_local_e = coordinate_transformation(drone1.id, GATE_FRONT_COOR[1], GATE_FRONT_COOR[0], 0)
    print(f"[{drone1.name}] Moving to N={wp1_local_n:.2f}, E={wp1_local_e:.2f}, D={hover_pos[2]:.2f}")
    end_heading1 = await straight_maneuver(drone1, 
                                           hover_pos[0], hover_pos[1], hover_pos[2],
                                           current_heading, 
                                           gate_front_local_n, gate_front_local_e, hover_pos[2], 
                                           2.0, 0.8)
    print(f"[{drone1.name}] +++ End 1st Mission +++")
    print(f"[{drone1.name}] Holding at (1.5, 5.0), wait for follower's reaction")
    await asyncio.sleep(5)

    print(f"[{drone1.name}] +++ Start 2nd Mission: Move to (19.0, 5.0) +++")
    ...
    end_heading2 = await straight_maneuver(drone1, 
                                           gate_front_local_n, gate_front_local_e, hover_pos[2],
                                           end_heading1,
                                           wp1_local_n, wp1_local_e, hover_pos[2],
                                           2.0, 0.8)
    ...
    while True:
        await drone1.set_position(
            wp1_local_n, wp1_local_e, hover_pos[2], end_heading2
        )
...
async def drone2_behavior(drone1, drone2):
    ...
    while True:
        ...
        # Additional velocity, due to gate
        gate_vel = compute_gate_passing_velocity(drone,my_pos,i)

        # Integrate velocity to get desired position
        next_pos = drone2_pos + (desired_vel + gate_vel) * UPDATE_RATE * P
        await drone2.set_position(next_pos[0], next_pos[1], hover_pos[2], 0.0)

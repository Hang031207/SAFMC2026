...
def coordinate_transformation(p1_n, p1_e, p2_n, p2_e, p3_n, p3_e):
    vec_1to2 = [p2_n-p1_n, p2_e-p1_e]
    vec_1to3 = [p3_n-p1_n, p3_e-p1_e]
    vec_3to2 = [ vec_1to2[0]-vec_1to3[0], vec_1to2[1]-vec_1to3[1] ]
    return vec_3to2

async def straight_maneuver(drone, start_n, start_e, start_d, start_heading, end_n, end_e, end_d, end_heading, v_default):
    ...
    while travelled < distance:
        ...
        step = min(step_size_default, 0.3*remaining)
        travelled += step

        cur_n = start_n + travelled * dn
        cur_e = start_e + travelled * de
        cur_d = start_d + travelled * dd

        cur_h = start_heading + delta_h * (travelled/distance)
        if cur_h < 0:
            cur_h += 360
        elif cur_h >= 360:
            cur_h -= 360
        ...
        iteration += 1
        if iteration % 10 == 0:
            print(f"[Leader] Heartbeart during Maneuver: N={cur_n:.2f}, E={cur_e:.2f}, D={cur_d:.2f}, Heading={cur_h:.2f}")
...
async def leader_behavior(leader,target_heading,id):
    ...
    if id==1:
        gate_front_local = coordinate_transformation(0, 0, GATE_FRONT_COOR[1], GATE_FRONT_COOR[0], DRONE_1_COOR[1], DRONE_1_COOR[0])
        gate_front_local_n = gate_front_local[0]
        gate_front_local_e = gate_front_local[1]
    elif id==2:
        gate_front_local = coordinate_transformation(0, 0, GATE_FRONT_COOR[1], GATE_FRONT_COOR[0], DRONE_2_COOR[1], DRONE_2_COOR[0])
        gate_front_local_n = gate_front_local[0]
        gate_front_local_e = gate_front_local[1]
    elif id==3:
        gate_front_local = coordinate_transformation(0, 0, GATE_FRONT_COOR[1], GATE_FRONT_COOR[0], DRONE_3_COOR[1], DRONE_3_COOR[0])
        gate_front_local_n = gate_front_local[0]
        gate_front_local_e = gate_front_local[1]
    # Wait for position updates to stabilize
    await asyncio.sleep(2)
    
    print(f"[LEADER] Leader assigned to drone {id}")
    ...
    # Start moving to the start gate front (1.5,5):
    ...
    if id==1:
        endpoint1_local = coordinate_transformation(0, 0, WAYPOINT1[1], WAYPOINT1[0], DRONE_1_COOR[1], DRONE_1_COOR[0])
        endpoint1_local_n = endpoint1_local[0]
        endpoint1_local_e = endpoint1_local[1]
    elif id==2:
        endpoint1_local = coordinate_transformation(0, 0, WAYPOINT1[1], WAYPOINT1[0], DRONE_2_COOR[1], DRONE_2_COOR[0])
        endpoint1_local_n = endpoint1_local[0]
        endpoint1_local_e = endpoint1_local[1]
    elif id==3:
        endpoint1_local = coordinate_transformation(0, 0, WAYPOINT1[1], WAYPOINT1[0], DRONE_3_COOR[1], DRONE_3_COOR[0])
        endpoint1_local_n = endpoint1_local[0]
        endpoint1_local_e = endpoint1_local[1]
    # Start mission 1: Fly pass the start gate: stop at the front of first gate
    print("\n[Leader] Start mission 1, straight maneuver\n"
          f"[Leader] Current position: N={gate_front_local_n:.2f}, E={gate_front_local_e:.2f}, D={hover_pos['d']:.2f}\n"
          f"Target endpoint: N={endpoint1_local_n:.2f}, E={endpoint1_local_e:.2f}, D={hover_pos['d']:.2f}\n"
          f"Target heading: {current_heading:.1f}°\n")
    await straight_maneuver(leader,
                            gate_front_local_n, gate_front_local_e, hover_pos['d'], current_heading,
                            endpoint1_local_n, endpoint1_local_e, hover_pos['d'], current_heading,
                            0.5 #m/s
    )
    ...
    while True:
        await leader.set_position(
            WAYPOINT[1], WAYPOINT[0], hover_pos['d'], current_heading
        )
...
async def follower_behavior(leader, follower,id,i):
    ...
    while True:
        ...
        # PHASE 5: TRACK - Follow leader closely in the maneuver
        elif phase == 5:
            ...
            await follower.set_position(
                ...
                new_heading  # Keep current heading
            )
            if (iteration * UPDATE_RATE) >= WAIT_TIME and heading_error < HEADING_TOLERANCE and pos_error < POSITION_TOLERANCE and height_error < HEIGHT_TOLERANCE:
                print(f"[FOLLOWER {i} PHASE 5] Follower can just do a perfect tracking!")
                phase = 6

        # Undefined phase: TRACK - Maintain formation
        else:
